...
export default function EditReview(){
    ...
    useEffect(() => {
        axios.post("https://adt-26-backend.onrender.com/getBook", {bookID: bookID})
        .then((res) => {
            setBook(res.data[0]);
        ...
        axios.post("https://adt-26-backend.onrender.com/getReview", {reviewID: reviewID})
        .then((res) => {
    ...
    return <Container className="py-5">
        ...
        <button className="btn btn-primary mx-4" onClick={() => {
                                            axios.post("https://adt-26-backend.onrender.com/editReview", {review: userReview, rating: userRating, reviewID: reviewID});
                                            navigate("/home", { state: { userEmail: userEmail, userName: userName } })  
                                        }} >Submit </button>
        <button className="btn btn-danger mx-4" onClick={() => {
                                            axios.post("https://adt-26-backend.onrender.com/deleteReview", {reviewID: reviewID});
                                            navigate("/home", { state: { userEmail: userEmail, userName: userName } })  
